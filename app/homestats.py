...
def homestats_sys(startTime):
    result={}
    av1, av2, av3 = os***REMOVED***getloadavg()
    av1 = round(av1,2)
    av2 = round(av2,2)
    av3 = round(av3,2)
    cpu_percent = psutil***REMOVED***cpu_percent()
    memory_percent = psutil***REMOVED***virtual_memory()***REMOVED***percent
    bootDelta = int(time***REMOVED***time() - psutil***REMOVED***boot_time())
    processDelta = int(time***REMOVED***time() - startTime)
    boot_uptime = homestats_uptime(bootDelta)
    process_uptime = homestats_uptime(processDelta)
    result***REMOVED***update({'av1': av1, 'av2': av2, 'av3': av3, 'cpu_percent': cpu_percent, 'memory_percent': memory_percent,
...
def homestats_runtime():
    result = {}
    python_version = str(sys***REMOVED***version_info***REMOVED***major)+'***REMOVED***'+str(sys***REMOVED***version_info***REMOVED***minor)
    unison_version = subprocess***REMOVED***check_output(['unison', '-version'])***REMOVED***split()[2]***REMOVED***decode('utf-8')
    flask_version = flask***REMOVED***__version__
...
def homestats_uptime(seconds):
    result = []
    #uptime_seconds=int(time***REMOVED***time() - seconds)
