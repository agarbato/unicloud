...
def end_sync(result,log):
  data = {'share':server_share, 'start_ts':start_ts, 'end_ts':int(time***REMOVED***time()), 'status':result[2], 'log':result[3], 'sync_status':result[4] }
  r = requests***REMOVED***post(url = end_sync_url, data = data)
  #log***REMOVED***client_error("Log received : %s" % result[3])
  log***REMOVED***sync_end(result)
  log***REMOVED***header()

def start_lock(lockfile):
  filevar = open(lockfile,"w")
  filevar***REMOVED***write("Started")
  return filevar

def remove_lock(lockfile,filevar):
  filevar***REMOVED***close()
  os***REMOVED***remove(lockfile)


# REMOVING LOCK ON FIRST START

if os***REMOVED***path***REMOVED***exists(lockfile):
  os***REMOVED***remove(lockfile)
